...
/// A 8-bit timer with two output compare units.
pub trait Timer8O2 : Sized {
    ...
    const OCIEB: RegisterBits<Self::InterruptMask>;
}
...
impl ClockSource {
    fn bits<T: Timer8O2>(&self) -> RegisterBits<T::ControlB> {
    ...
    #[inline]
    fn mask<T: Timer8O2>() -> RegisterBits<T::ControlB> {
        !(T::CS2 | T::CS1 | T::CS0)
    }
}
...
impl WaveformGenerationMode {
    ...
    #[inline]
    fn bits<T: Timer8O2>(&self) -> (RegisterBits<T::ControlA>, RegisterBits<T::ControlB>) {
    ...
    #[inline]
    fn mask<T: Timer8O2>() -> (RegisterBits<T::ControlA>, RegisterBits<T::ControlB>) {
        (!(T::WGM0 | T::WGM1), !(T::WGM2))
    }
}

pub struct Timer8O2Setup<T: Timer8O2> {
    a: RegisterBits<T::ControlA>,
    b: RegisterBits<T::ControlB>,
    output_compare_1: Option<u8>,
    output_compare_2: Option<u8>,
    _phantom: marker::PhantomData<T>,
}

impl<T: Timer8O2> Timer8O2Setup<T> {
    #[inline]
    pub fn new() -> Self {
        Timer8O2Setup {
            a: RegisterBits::zero(),
    ...
    pub fn configure(self) {
        ...
        if let Some(v) = self.output_compare_2 {
            // Set the match
            T::CompareB::write(v);

            // Enable compare interrupt
            T::InterruptMask::set(T::OCIEB);
        }
    }
}
